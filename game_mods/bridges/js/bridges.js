...
function initGameProcess() {
    ...
    gameButton.addEventListener('click', function () {
        if (buttonStatus === 1) { //При нажатии на "Подтвердить"
            ...
            playerChoice.setIcon('img/bridges.png')
            markers[roundCounter - 1].setIcon('img/bridges.png')
            updateRound();
            if(line){
                line.setMap(null)
            }
        ...
        if (zoom < 12) {
            for (let i = 0; i < markers.length; i++) {
                ...
                markers[i].addListener("click", function (event) {
                    ...
                    markers[i].setIcon('img/bridge.png');
                    prevMarker = i;
                });
            }
        } else {
...
function updateScore() {
    if (playerChoice.getTitle() === locations[roundCounter].name) {
        ...
        playerChoice.setIcon('img/bridgeGood.png')
        listener.remove();
    }
    else {
        listener.remove();
        ...
        playerChoice.setIcon('img/bridgeBad.png');
        markers[roundCounter].setIcon('img/bridgeGood.png');
    }
}
